...
formal: annotations? name = IDENTIFIER;
...
basicExpr: THIS | SUPER | IDENTIFIER | literal;
...
unaryStringExpr: STRLEN LPAREN op = expression RPAREN;
...
arrayExpr: ARRAYLEN LPAREN op = expression RPAREN;
...
loop: (IDENTIFIER COLON)? (forLoop | whileLoop);

forLoop: FOR LPAREN forDeclaration RPAREN blockOrStatement;

whileLoop: WHILE parExpr blockOrStatement;

forDeclaration
	: (initDecl = localDeclaration | initExpr = expression)? SEMI condition = expression? SEMI post
		= expression?
;
...
annotation: name = unitName annotationMembers?;

annotationMembers
	: LPAREN annotationMember (COMMA annotationMember)* RPAREN
;

annotationMember: IDENTIFIER ASSIGN annotationValue;

annotationValue: basicAnnotationValue | arrayAnnotationValue;
...
annotations: LBRACK annotation (COMMA annotation)* RBRACK;

/*
 * BLOCK
 */

block: LBRACE blockOrStatement* RBRACE;
...
fieldDeclaration: annotations? name = IDENTIFIER SEMI;
...
unit: classUnit | interfaceUnit;

classUnit
	: annotations? ABSTRACT? CLASS name = unitName (
		EXTENDS superclass = unitName
	)? (IMPLEMENTS superinterfaces = unitNames)? LBRACE declarations = classMemberDeclarations
		RBRACE
;

interfaceUnit
	: annotations? INTERFACE name = unitName (
		EXTENDS superinterfaces = unitNames
	)? LBRACE declarations = interfaceMemberDeclarations RBRACE
;

unitNames: unitName (COMMA unitName)*;
