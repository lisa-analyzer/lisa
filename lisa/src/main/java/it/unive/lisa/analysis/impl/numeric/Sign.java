		...
		case COMPARISON_LT:
			if (left instanceof Identifier) {
				...
				if (rightSign.isNegative() || rightSign.isZero()) // x < 0/-
					environment = environment.assign((Identifier) left, new Constant(right.getDynamicType(), -1, right.getLocation()), pp);
			} else if (right instanceof Identifier) {
				Sign leftSign = eval(left, environment, pp);
				if (leftSign.isPositive() || leftSign.isZero()) // 0/+ < x
					environment = environment.assign((Identifier) right, new Constant(left.getDynamicType(), 1, left.getLocation()), pp);
			}
			return environment;
		case COMPARISON_GT:
			if (left instanceof Identifier) {
				...
				if (leftSign.isNegative() || leftSign.isZero()) // -/0 > x
					environment = environment.assign((Identifier) right, new Constant(left.getDynamicType(), -1, right.getLocation()), pp);
			}
			return environment;
		default:
			return environment;
