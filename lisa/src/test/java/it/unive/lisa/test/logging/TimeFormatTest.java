...
public class TimeFormatTest {
	...
	public void testUpToMinutes() {
		loopRunner(
				l -> {
					long minutes = TimeUnit.NANOSECONDS.toMinutes(l);
					long seconds = TimeUnit.NANOSECONDS.toSeconds(l - TimeUnit.MINUTES.toNanos(minutes));
					long millis = TimeUnit.NANOSECONDS.toMillis(l - TimeUnit.MINUTES.toNanos(minutes) - TimeUnit.SECONDS.toNanos(seconds));
					long nanos = l - TimeUnit.MINUTES.toNanos(minutes) - TimeUnit.SECONDS.toNanos(seconds) - TimeUnit.MILLISECONDS.toNanos(millis);
					String result = nanos + UNIT_NANOS;
					if (millis != 0)
						result = millis + UNIT_MILLIS + " " + result;
					if (seconds != 0)
	...
	public void testUpToHours() {
		loopRunner(
				l -> {
					...
					long seconds = TimeUnit.NANOSECONDS.toSeconds(l - TimeUnit.HOURS.toNanos(hours) - TimeUnit.MINUTES.toNanos(minutes));
					long millis = TimeUnit.NANOSECONDS.toMillis(l - TimeUnit.HOURS.toNanos(hours) - TimeUnit.MINUTES.toNanos(minutes) - TimeUnit.SECONDS.toNanos(seconds));
					long nanos = l - TimeUnit.HOURS.toNanos(hours) - TimeUnit.MINUTES.toNanos(minutes) - TimeUnit.SECONDS.toNanos(seconds) - TimeUnit.MILLISECONDS.toNanos(millis);
					String result = nanos + UNIT_NANOS;
					if (millis != 0)
						result = millis + UNIT_MILLIS + " " + result;
					if (seconds != 0)
	...
	private void loopRunner(Function<Long, String> expectedGenerator, Function<Long, String> actualGenerator) {
		...
		LongStream.iterate(1 + generator.nextInt(9), l -> l = (1 + generator.nextInt(9)) * ((long) Math.pow(10, Math.log10(l))) + l)
