...
public class ImplementedCFG extends CodeGraph<ImplementedCFG, Statement, Edge> implements CodeMember, CFG {
	...
	public final <A extends AbstractState<A, H, V, T>,
		...
		Fixpoint<ImplementedCFG, Statement, Edge,
	...
	private final class CFGFixpoint<A extends AbstractState<A, H, V, T>,
		...
		private CFGFixpoint(int widenAfter, InterproceduralAnalysis<A, H, V, T> interprocedural) {
			...
			this.lubs = new HashMap<>(ImplementedCFG.this.getNodesCount());
		}
		...
		public Pair<AnalysisState<A, H, V, T>, StatementStore<A, H, V, T>> traverse(Edge edge,
			...
			for (VariableTableEntry entry : ImplementedCFG.this.descriptor.getVariables())
				if (entry.getScopeEnd() == edge.getSource())
					toRemove.add(entry);

			Collection<Identifier> ids = new LinkedList<>();
			for (VariableTableEntry entry : toRemove) {
				SymbolicExpression v = entry.createReference(ImplementedCFG.this).getVariable();
	...
	public ProgramPoint getGenericProgramPoint() {
		return new ProgramPoint() {
			...
			public ImplementedCFG getCFG() {
				return ImplementedCFG.this;
			}

			@Override
			public String toString() {
				return "unknown program point in " + ImplementedCFG.this.getDescriptor().getSignature();
			}
