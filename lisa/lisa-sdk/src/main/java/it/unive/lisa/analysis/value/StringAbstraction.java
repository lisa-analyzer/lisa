...
/**
 * A {@link ValueDomain} that focuses on string values. This interface provides
 * a default implementation of
 * {@link #canProcess(ValueExpression, ProgramPoint, SemanticOracle)} that
 * checks whether the given expression is string or not.
 * 
 * @author <a href="mailto:luca.negrini@unive.it">Luca Negrini</a>
 * 
 * @param <L> the type of lattice used by this domain
 */
public interface StringAbstraction<
	...
	default boolean canProcess(
		...
		if (expression instanceof PushInv)
			...
			return whole ? expression.getStaticType().isStringType() : expression.getStaticType().isValueType();
		...
		if (whole)
			return rts.stream().anyMatch(Type::isStringType);
		else
