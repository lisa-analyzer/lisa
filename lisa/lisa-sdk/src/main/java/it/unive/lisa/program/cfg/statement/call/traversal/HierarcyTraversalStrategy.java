 ...
 * iterator returning {@link CompilationUnit}s in the order they should be
 * visited.
 * 
...
public interface HierarcyTraversalStrategy {
	...
	/**
	 * Yields an iterator for traversing a type hierarchy to find call targets.
	 * 
	 * @param call  the call for which the traversal is requested
	 * @param start the unit where the traversal should start
	 * 
	 * @return an iterator that returns the units in order in which they must be
	 *             visited
	 */
	Iterator<CompilationUnit> traverse(Call call, CompilationUnit start);
}
