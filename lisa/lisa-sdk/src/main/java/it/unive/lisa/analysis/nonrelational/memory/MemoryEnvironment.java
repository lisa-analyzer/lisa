...
/**
 * An {@link Environment} that is also a {@link MemoryLattice}, tracking
 * locations pointed by variables and memory locations.
 * 
 ...
 *                this environment
 */
public class MemoryEnvironment<L extends MemoryValue<L>>
		extends
		FunctionalLattice<MemoryEnvironment<L>, Identifier, L>
		implements
		MemoryLattice<MemoryEnvironment<L>> {
	...
	@Override
	public Pair<MemoryEnvironment<L>, List<MemoryReplacement>> pushScope(
			ScopeToken scope,
			ProgramPoint pp)
			throws SemanticException {
		AtomicReference<SemanticException> holder = new AtomicReference<>();

		Pair<MemoryEnvironment<L>, List<MemoryReplacement>> result = liftIdentifiers(id -> {
			try {
	...
	@Override
	public Pair<MemoryEnvironment<L>, List<MemoryReplacement>> popScope(
			ScopeToken scope,
			ProgramPoint pp)
			throws SemanticException {
		AtomicReference<SemanticException> holder = new AtomicReference<>();
	...
	@Override
	public Pair<MemoryEnvironment<L>, List<MemoryReplacement>> forgetIdentifier(
			Identifier id,
			ProgramPoint pp)
			throws SemanticException {
	...
	@Override
	public Pair<MemoryEnvironment<L>, List<MemoryReplacement>> forgetIdentifiers(
	...
	@Override
	public Pair<MemoryEnvironment<L>, List<MemoryReplacement>> forgetIdentifiersIf(
	...
	@Override
	public MemoryEnvironment<L> top() {
		return isTop() ? this : new MemoryEnvironment<>(lattice.top(), null);
	}

	@Override
	public MemoryEnvironment<L> bottom() {
		return isBottom() ? this : new MemoryEnvironment<>(lattice.bottom(), null);
	}
