...
/**
 * An ExpressionVisitor that normalizes boolean expressions into canonical
 * forms to be used in the octagon domain numeric analysis
 * It implements the following normalizations:
 * 
 ...
 * </ul>
 * 
 * <h3>3. Special handling for integer types</h3>
 * For integer numerical domains (when {@code type.isInteger() == true}):
 * <ul>
...
public class BooleanExpressionNormalizer implements ExpressionVisitor<SymbolicExpression> {
     ...
     */
    public BooleanExpressionNormalizer(CodeLocation location, ProgramPoint pp,
     ...
     * @param location the current program point
     */
    public BooleanExpressionNormalizer(CodeLocation location) {
        this.location = location;
        this.pp = null;
        this.oracle = null;
    }
     ...
     */
    public static SymbolicExpression normalize(SymbolicExpression expression, CodeLocation location,
    ...
    private UnaryExpression neg(SymbolicExpression e) {
        return new UnaryExpression(e.getStaticType(), e, NumericNegation.INSTANCE, location);
    }

    private BinaryExpression add(SymbolicExpression a, SymbolicExpression b) {
        return new BinaryExpression(a.getStaticType(), a, b, NumericNonOverflowingAdd.INSTANCE, location);
    }

    private BinaryExpression sub(SymbolicExpression a, SymbolicExpression b) {
     ...
     */
    private SymbolicExpression simplifiedSub(SymbolicExpression a, SymbolicExpression b) {
    ...
    private SymbolicExpression normalizeLeZeroLeft(SymbolicExpression left) throws SemanticException {
    ...
    private boolean shouldTightenForInteger(SymbolicExpression constantExpr) throws SemanticException {
    ...
    private BinaryExpression cmp(SymbolicExpression l, SymbolicExpression r, BinaryOperator op, Type resultType) {
        return new BinaryExpression(resultType, l, r, op, location);
    }

    private boolean isIntegerPair(SymbolicExpression a, SymbolicExpression b) throws SemanticException {
        return isIntegerType(a) && isIntegerType(b);
    }

    @Override
    public SymbolicExpression visit(BinaryExpression expression, SymbolicExpression left, SymbolicExpression right,
        ...
        if (operator instanceof ComparisonGt) {
            ...
            if (left instanceof BinaryExpression && right instanceof Constant) {
                ...
                if (leftBinExpr.getOperator() instanceof NumericNonOverflowingAdd) {
                    ...
                    if (isIntegerType(left) && isIntegerType(rightConst)) {
                        // For integers: (x + y) > c becomes -(x + y) + (c + 1) <= 0
                        ...
                        // For non-integers: distribute negation over addition to match expected shape
                        // (x + y) > c => (-x + -y [+ c]) <= 0
                        ...
                        // If c != 0, keep it on the left side as +c; for c == 0, avoid adding +0
                    ...
                    // (x - y) > c becomes (x - y) - c > 0, then normalize as -(x - y - c) < 0
                    ...
                    if (isIntegerType(left) && isIntegerType(rightConst)) {
                        // General case for integers: expr > c becomes -expr + (c + 1) <= 0
                        Constant incrementedConst = inc(rightConst, 1);
            ...
            if (right instanceof Constant && ((Constant) right).getValue().equals(new MathNumber(0))) {
                // Handle (-a - b) >= 0 -> (a + b) <= 0 (first to avoid the generic e1-e2
                // case)
                ...
                // Handle (-a + b) >= 0 -> (a - b) <= 0 and (a + -b) >= 0 -> (b - a) <= 0
                if (left instanceof BinaryExpression
            ...
            // a < b: for integers keep LT (will become +1 <= 0), for non-integers relax to
            // LE
            ...
            // If right is exactly 0, keep the left side as-is to avoid creating (left - 0)
            if (right instanceof Constant && ((Constant) right).getValue().equals(new MathNumber(0))) {
     ...
     * @return the normalized expression
     */
    private SymbolicExpression createNormalizedComparison(SymbolicExpression left, SymbolicExpression right,
            BinaryOperator comparison, Type resultType, boolean isInteger) {
     ...
     */
    private SymbolicExpression createNormalizedComparison(SymbolicExpression left, SymbolicExpression right,
            BinaryOperator comparison, Type resultType) throws SemanticException {
    ...
    /**
     * Creates the normalization for equality: a == b becomes
     * (a - b <= 0) AND (-a + b <= 0).
     * 
     ...
     * @throws SemanticException if an error occurs during normalization
     */
    private SymbolicExpression createEqualityNormalization(SymbolicExpression left, SymbolicExpression right,
            Type resultType) throws SemanticException {
    ...
    /**
     * Creates the normalization for inequality: a != b.
     * For integer types: (a - b + 1 <= 0) OR (-a + b + 1 <= 0)
     * For real types: keeps the original expression or uses different approaches
     * 
     * @param left       the first operand
     * @param right      the second operand
     * @param resultType the resulting type of the expression
     ...
     * @throws SemanticException if an error occurs during normalization
     */
    private SymbolicExpression createInequalityNormalization(SymbolicExpression left, SymbolicExpression right,
            Type resultType) throws SemanticException {
        ...
        if (isInteger) {
            ...
            // Try to fold constants: if left or right are constants or the subtraction
            // already contains a constant on the right, merge the +1 into that constant.

            // Build leftMinusRight + 1 but fold where possible
            SymbolicExpression leftMinusRight = simplifiedSub(left, right);
            SymbolicExpression leftCond;
            // If subtraction is of the form (e - c) with c constant, fold: e - (c - 1)
            if (leftMinusRight instanceof BinaryExpression
                ...
                // both constants: compute c1 - c2 + 1 and produce a single constant comparison
                MathNumber v1 = (MathNumber) ((Constant) left).getValue();
                MathNumber v2 = (MathNumber) ((Constant) right).getValue();
                Constant foldedConst = new Constant(left.getStaticType(), v1.subtract(v2).add(new MathNumber(1)),
                        location);
     ...
     */
    private SymbolicExpression pushNegationInward(SymbolicExpression expression) throws SemanticException {
        ...
        if (expression instanceof BinaryExpression) {
            ...
            // We recognize specific patterns for octagon domains
                ...
                // Special pattern: not(A - B <= 0) -> B - A + 1 <= 0 (for integers) or B - A
                // <= 0 (for reals)
                if (left instanceof BinaryExpression
                        && ((BinaryExpression) left).getOperator() instanceof NumericNonOverflowingSub
                    ...
                    if (isIntegerType(A) && isIntegerType(B)) {
                        ...
                        // If diff has a constant on the right: (e - c) + 1 -> e - (c - 1)
                        ...
                        // If diff has a constant on the left: (c - e) + 1 -> (c + 1) - e
                        if (diff instanceof BinaryExpression
                ...
                // Special pattern: not(A - B + 1 <= 0) -> B - A <= 0 (to handle not(x < y))
                if (left instanceof BinaryExpression) {
                ...
                if (isIntegerPair(left, right)) {
                    ...
                    // Special case: if right is a constant, use (-x + (c+1)) form instead of (c+1
                    // -
                    // x)
                    ...
                    // Special case: if right is a constant, use (-x + c) form instead of (c - x)
                    if (right instanceof Constant) {
                        SymbolicExpression negLeft = neg(left);
                ...
                // not(e1 >= e2) -> e1 < e2
                // For integers keep strict inequality (will be converted to +1 <= 0),
     ...
     */
    private boolean isIntegerType(SymbolicExpression e) throws SemanticException {
    ...
    @Override
    public SymbolicExpression visit(UnaryExpression expression, SymbolicExpression arg, Object... params)
            throws SemanticException {
        ...
        if (expression.getOperator() instanceof LogicalNegation) {
            ...
            // not the already normalized one (arg), to preserve the correct form
            // of cases like not(x < y) on non-integer types (-> <=) and not(x == y) (->
            // !=).
    ...
    @Override
    public SymbolicExpression visit(TernaryExpression expression, SymbolicExpression left, SymbolicExpression middle,
            SymbolicExpression right, Object... params) throws SemanticException {
    ...
    @Override
    public SymbolicExpression visit(HeapExpression expression, SymbolicExpression[] subExpressions, Object... params)
            throws SemanticException {
        return expression;
    }

    @Override
    public SymbolicExpression visit(AccessChild expression, SymbolicExpression receiver, SymbolicExpression child,
            Object... params) throws SemanticException {
        return expression;
    }

    @Override
    public SymbolicExpression visit(MemoryAllocation expression, Object... params) throws SemanticException {
        return expression;
    }

    @Override
    public SymbolicExpression visit(HeapReference expression, SymbolicExpression arg, Object... params)
            throws SemanticException {
        return expression;
    }

    @Override
    public SymbolicExpression visit(HeapDereference expression, SymbolicExpression arg, Object... params)
            throws SemanticException {
        return expression;
    }

    @Override
    public SymbolicExpression visit(ValueExpression expression, SymbolicExpression[] subExpressions, Object... params)
            throws SemanticException {
        return expression;
    }

    @Override
    public SymbolicExpression visit(Skip expression, Object... params) throws SemanticException {
        return expression;
    }

    @Override
    public SymbolicExpression visit(PushAny expression, Object... params) throws SemanticException {
        return expression;
    }

    @Override
    public SymbolicExpression visit(PushInv expression, Object... params) throws SemanticException {
        return expression;
    }

    @Override
    public SymbolicExpression visit(Constant expression, Object... params) throws SemanticException {
        return expression;
    }

    @Override
    public SymbolicExpression visit(Identifier expression, Object... params) throws SemanticException {
        return expression;
    }
}