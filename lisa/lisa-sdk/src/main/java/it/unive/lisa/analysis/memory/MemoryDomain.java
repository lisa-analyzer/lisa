 ...
 * on memory locations, and not on concrete values. A memory domain can handle
 * instances of {@link MemoryExpression}s. Transformers of this domain yield an
 * abstract state and the substitutions induced by the transformer itself. The
 * substitutions, in the form of a list of {@link MemoryReplacement}s that
 * <b>must</b> be processed in their order of appearance, that the creation of
 * this memory domain caused. Each substitution maps {@link Identifier}s in the
 ...
 * 
 * @param <L> the type of {@link MemoryLattice} that this domain works with
 */
public interface MemoryDomain<L extends MemoryLattice<L>>
		extends
		SemanticComponent<L, Pair<L, List<MemoryReplacement>>, SymbolicExpression, Identifier> {
			...
			for (SymbolicExpression id : ws) {
				...
				for (SymbolicExpression r : rewritten) {
					if (r instanceof MemoryPointer) {
						MemoryLocation l = ((MemoryPointer) r).getReferencedLocation();
						locs.add(l);
						result.add(l);
					} else
						locs.add(r);
	...
	/**
	 * A replacement between {@link Identifier}s caused by a change in the
	 * memory abstraction. A replacement express a relation between two sets of
	...
	class MemoryReplacement {
		 ...
		 */
		public MemoryReplacement withSource(
				Identifier id) {
			addSource(id);
			return this;
		}
		 ...
		 * @return this replacement
		 */
		public MemoryReplacement withTarget(
				Identifier id) {
			addTarget(id);
			return this;
		}
		...
		public boolean equals(
			...
			if (getClass() != obj.getClass())
				return false;
			MemoryReplacement other = (MemoryReplacement) obj;
			if (sources == null) {
