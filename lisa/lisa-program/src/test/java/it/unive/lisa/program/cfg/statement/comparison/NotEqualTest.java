...
public class NotEqualTest {
	...
	private NotEqual notEqual() {
		...
		return new NotEqual(TestFixtures.CFG, TestFixtures.LOCATION, left, right);
	}

	@Test
	public void unconditionallyBuildsTheInequalityOperatorAndDelegatesToSmallStepSemantics()
			throws SemanticException {
		// unlike the ordering comparisons (<, <=, >, >=), inequality places no
		...
		AnalysisState<UnitLattice> result = ne.fwdBinarySemantics(
		...
		assertEquals(ComparisonNe.INSTANCE, built.getOperator());
		...
		assertEquals(ne.getLocation(), built.getCodeLocation());
