...
/**
 * Float32 variants of BooleanExpressionNormalizer tests. Expectations are
 * adapted for non-integer semantics (no +1 offsets, use < where applicable).
 * 
...
public class BooleanExpressionNormalizerFloat32Test {
	...
	@Test // not(x >= 0) => x <= 0
	// not(x >= 0) is equivalent to x < 0, but in float case we need to relax to
	// x
