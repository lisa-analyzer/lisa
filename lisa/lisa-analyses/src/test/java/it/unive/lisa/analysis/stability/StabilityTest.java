...
public class StabilityTest {
	...
	Stability<Interval> s1top = new Stability<>(
			new ValueEnvironment<>(new Interval()).putState(x, interval1),
			new ValueEnvironment<>(new Trend()).putState(x, trendTOP));

	Stability<Interval> s1bot = new Stability<>(
			new ValueEnvironment<>(new Interval()).putState(x, interval1),
			new ValueEnvironment<>(new Trend()).putState(x, trendBOTTOM));

	Stability<Interval> s1st = new Stability<>(
			new ValueEnvironment<>(new Interval()).putState(x, interval1),
			new ValueEnvironment<>(new Trend()).putState(x, trendSTABLE));

	Stability<Interval> s1in = new Stability<>(
			new ValueEnvironment<>(new Interval()).putState(x, interval1),
			new ValueEnvironment<>(new Trend()).putState(x, trendINC));

	Stability<Interval> s1de = new Stability<>(
			new ValueEnvironment<>(new Interval()).putState(x, interval1),
			new ValueEnvironment<>(new Trend()).putState(x, trendDEC));

	Stability<Interval> s1nd = new Stability<>(
			new ValueEnvironment<>(new Interval()).putState(x, interval1),
			new ValueEnvironment<>(new Trend()).putState(x, trendNON_DEC));

	Stability<Interval> s1ni = new Stability<>(
			new ValueEnvironment<>(new Interval()).putState(x, interval1),
			new ValueEnvironment<>(new Trend()).putState(x, trendNON_INC));

	Stability<Interval> s1ns = new Stability<>(
			new ValueEnvironment<>(new Interval()).putState(x, interval1),
			new ValueEnvironment<>(new Trend()).putState(x, trendNON_STABLE));

	Stability<Interval> s2top = new Stability<>(
			new ValueEnvironment<>(new Interval()).putState(x, interval2),
			new ValueEnvironment<>(new Trend()).putState(x, trendTOP));

	Stability<Interval> s2bot = new Stability<>(
			new ValueEnvironment<>(new Interval()).putState(x, interval2),
			new ValueEnvironment<>(new Trend()).putState(x, trendBOTTOM));

	Stability<Interval> s2st = new Stability<>(
			new ValueEnvironment<>(new Interval()).putState(x, interval2),
			new ValueEnvironment<>(new Trend()).putState(x, trendSTABLE));

	Stability<Interval> s2in = new Stability<>(
			new ValueEnvironment<>(new Interval()).putState(x, interval2),
			new ValueEnvironment<>(new Trend()).putState(x, trendINC));

	Stability<Interval> s2de = new Stability<>(
			new ValueEnvironment<>(new Interval()).putState(x, interval2),
			new ValueEnvironment<>(new Trend()).putState(x, trendDEC));

	Stability<Interval> s2nd = new Stability<>(
			new ValueEnvironment<>(new Interval()).putState(x, interval2),
			new ValueEnvironment<>(new Trend()).putState(x, trendNON_DEC));

	Stability<Interval> s2ni = new Stability<>(
			new ValueEnvironment<>(new Interval()).putState(x, interval2),
			new ValueEnvironment<>(new Trend()).putState(x, trendNON_INC));

	Stability<Interval> s2ns = new Stability<>(
			new ValueEnvironment<>(new Interval()).putState(x, interval2),
			new ValueEnvironment<>(new Trend()).putState(x, trendNON_STABLE));
	...
	public void test_Equals() {
		...
		Stability<Interval> s1ns_bis = new Stability<>(
				new ValueEnvironment<>(new Interval()).putState(x, interval1),
				new ValueEnvironment<>(new Trend()).putState(x, trendNON_STABLE));
	...
	public void test_LessOrEqual() throws SemanticException {
		...
		assertTrue(s2in.lessOrEqual(s1ns)); // {[3,4], INC} Le {[3,5],
											// NON_STABLE}
		assertTrue(s2in.lessOrEqual(s1in)); // {[3,4], INC} <= {[3,5], INC}
		assertTrue(s2in.lessOrEqual(s2ns)); // {[3,4], INC} <= {[3,4],
											// NON_STABLE}

		// interval <= interval && trend !<= trend
		assertFalse(s2ns.lessOrEqual(s1in)); // {[3,4], NON_STABLE} !<= {[3,5],
												// INC}
		...
		// interval !<= interval && trend !<=trend
		assertFalse(s1ns.lessOrEqual(s2in)); // {[3,5], NON_STABLE} !<= {[3,4],
												// INC}
	...
	/*
	 * @Test public void test_Lub() throws SemanticException {
	 * assertEquals(s1in, s1in.lub(s1in)); // lub({[3,5], INC}, {[3,5], DEC}) =
	 * {[3,5], NON_STABLE} assertEquals(s1ns.getIntervals(),
	 * s1in.getIntervals().lub(s1de.getIntervals()));
	 * //assertEquals(s1ns.getTrend().lattice,
	 * s1in.getTrend().lattice.lub(s1de.getTrend().lattice)); assertEquals(s1ns,
	 * s1in.lub(s1de)); assertEquals(s1ns, s1de.lub(s1in)); // lub({[3,5], INC},
	 * {[3,5], NON_STABLE}) = {[3,5], NON_STABLE} assertEquals(s1ns,
	 * s1in.lub(s1ns)); assertEquals(s1ns, s1ns.lub(s1in)); // lub({[3,5],
	 * NON_DEC}, {[3,5], DEC}) = {[3,5], TOP} assertEquals(s1top,
	 * s1nd.lub(s1de)); }
	 */
}
