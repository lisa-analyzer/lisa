/*
 * Mpfr.java APRON Library / Java GMP/MPFR binding Copyright (C) Antoine Mine'
 * 2010
 */
 ...
 * <p>
 * This class wraps {@code mpfr_t} MPFR objects.
 * <p>
 * The precision of each number is set at construction time and not changed when
 * the number is modified (except by {@link #setPrec} or {@link #set(Mpfr)}).
 * Thus, most operators entail rounding. Such operators take as (last) argument
 * a rounding direction that can be {@link #RNDN}, {@link #RNDZ}, {@link #RNDU},
 * or {@link #RNDD}).
 * <p>
 * Most operators return an integer that tells whether the stored result is
 * exact (zero returned), over-approximated (positive number returned), or
 * under-approximated (negative number returned).
 * <p>
 * In addition, the class has some global (static) exception flags that can be
 * set by methods to denote IEEE 754 exceptions, and then inspected and reset by
 * the user: overflow, underflow, NaN, inexact, and erange (invalid range) flags
 * (these do not cause Java exceptions to be thrown).
 */
public class Mpfr
		implements
		Serializable,
	...
	/**
	 * Constructs a float with the default precision equal to an integer (with
	 * rounding).
	 */
	public Mpfr(
			int v,
	...
	/**
	 * Constructs a float with the default precision equal to a double (with
	 * rounding).
	 */
	public Mpfr(
			double v,
	...
	/**
	 * Constructs a float with the default precision equal to a fraction (with
	 * rounding).
	 */
	public Mpfr(
			Mpq v,
	 ...
	 * <p>
	 * The default is 53 bits initially (i.e., same as double precision IEEE
	 * floats).
	 */
	static public native void setDefaultPrec(
			int prec);
	...
	/**
	 * Sets this to the number represented in the String s in base b (with
	 * rounding).
	 */
	...
	/**
	 * Sets this to the number represented in the decimal String s (with
	 * rounding).
	 */
	public void set(
			String s,
	 ...
	 * <p>
	 * The mantissa m returned is such that 0.5 &lt;= |m| &lt; 1, rounded to fit
	 * double.
	 * <p>
	 * The exponent is stored into r[0].
	 */
	public native double doubleExponentValue(
			int[] r,
			int round);

	/** Converts this to an integer (with rounding) and returns it. */
	public native Mpz MpzValue(
			int round);
	 ...
	 */
	public native int MpzExponentValue(
			Mpz r);
	 ...
	 * <p>
	 * n is the number of digits to output, or 0 to denote sufficient precision
	 * so that it is possible to recover the original value when parsing using
	 * rounding to nearest.
	 */
	...
	public int neg(
			int round) {
		return neg(this, round);
	}

	/** Sets this to |op|. */
	public native int abs(
			Mpfr op,
			int round);
	 ...
	 * A NaN causes erange to be set and 0 to be returned.
	 */
	public native int cmp2exp(
			int op,
			int e);

	/** Compares |this| to |op|. */
	public native int cmpAbs(
			Mpfr op);
	 ...
	 */
	public native boolean isGreater(
			Mpfr op);
	 ...
	 * false if any operand is NaN.
	 */
	public native boolean isGreaterEqual(
			Mpfr op);
	 ...
	 * false if any operand is NaN.
	 */
	public native boolean isLessEqual(
			Mpfr op);
	 ...
	 * false if any operand is NaN.
	 */
	public native boolean isLessGreater(
			Mpfr op);
	...
	public native boolean isUnordered(
			Mpfr op);

	/**
	 * Whether x is a Mpfr and has the same value as this.
	 * <p>
	 * This compares representations, and so, NaN is assumed equal to NaN
	 * (unlike {@link #isEqual} that follows the IEEE 754 convention).
	 */
	...
	public boolean isPhysicalEqual(
			Mpfr x) {
		return ptr == x.ptr;
	}
	...
	public native int log2(
			Mpfr op,
			int round);

	/** Sets this to the logarithm in base 10 of op. */
	public native int log10(
			Mpfr op,
			int round);

	/** Sets this to e^op. */
	public native int exp(
			Mpfr op,
			int round);

	/** Sets this to 2^op. */
	public native int exp2(
			Mpfr op,
			int round);

	/** Sets this to 10^op. */
	public native int exp10(
			Mpfr op,
			int round);

	/** Sets this to the cosine of op. */
	public native int cos(
			Mpfr op,
			int round);

	/** Sets this to the sine of op. */
	public native int sin(
			Mpfr op,
			int round);

	/** Sets this to the tangent of op. */
	public native int tan(
			Mpfr op,
			int round);
	...
	public native int erf(
			Mpfr op,
			int round);

	/** Sets this to the value of the complementary error function at op. */
	public native int erfc(
			Mpfr op,
			int round);

	/**
	 * Sets this to the value of the first kind Bessel function of order 0 at
	 * op.
	 */
	...
	/**
	 * Sets this to the value of the first kind Bessel function of order 1 at
	 * op.
	 */
	...
	/**
	 * Sets this to the value of the first kind Bessel function of order n at
	 * op.
	 */
	public native int jn(
			int n,
			Mpfr op,
			int round);

	/**
	 * Sets this to the value of the second kind Bessel function of order 0 at
	 * op.
	 */
	...
	/**
	 * Sets this to the value of the second kind Bessel function of order 1 at
	 * op.
	 */
	...
	/**
	 * Sets this to the value of the second kind Bessel function of order n at
	 * op.
	 */
	public native int yn(
			int n,
			Mpfr op,
			int round);

	/** Sets this to (op1*op2)+op3. */
	public native int fma(
			Mpfr op1,
			Mpfr op2,
			Mpfr op3,
			int round);
	 ...
	 * <p>
	 * Assuming that this approximates some number x in the direction round1
	 * with error at most 2^(expb-err), where expb is the exponent of b, returns
	 * true if x can be correctly rounded to prec bits in the direction round2.
	 * Does not modify this.
	 */
	public native boolean canRound(
			int err,
			int round1,
			int round2,
			int prec);
	...
	static public native String toStringRndMode(
			int round);
	...
	/**
	 * Sets this to the next representable float following x in the direction of
	 * y.
	 */
	public native void nextToward(
			Mpfr op);
	...
	public native int setSign(
			Mpfr op,
			boolean s,
			int round);

	/**
	 * Sets this to op1 (rounded), and then sets the bit-sign of this to that of
	 * op2.
	 */
	public native int copySign(
			Mpfr op1,
			Mpfr op2,
			int round);
	 ...
	 */
	static public native boolean setEmin(
			long exp);
	 ...
	 * Returns true if the change was successful.
	 */
	static public native boolean setEmax(
			long exp);
	...
	public native int checkRange(
			int t,
			int round);

	/** Rounds this to emulate subnormal arithmetic. */
	public native int subNormalize(
			int t,
			int round);
	 ...
	 * Serializes this.
	 * <p>
	 * Not only the value, but also the precision is stored.
	 */
	private void writeObject(
			ObjectOutputStream out)
			throws IOException {
		/*
		 * format: - precision - string in base 16
		 */
	 ...
	 * <p>
	 * Not only the value, but also the precision is restored.
	 */
