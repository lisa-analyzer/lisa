/*
 * Mpz.java APRON Library / Java GMP/MPFR binding Copyright (C) Antoine Mine'
 * 2010
 */
 ...
 * <p>
 * This class wraps {@code mpz_t} GMP objects.
 */
public class Mpz
		implements
		Serializable,
	 ...
	 * <p>
	 * The magnitude is the absolute value, and it is returned as a byte array
	 ...
	 *
	 * @param b must be either 0, or between 2 and 62. When b is 0, the leading
	 *              characters are used to determine the base (0x and 0X for
	 *              hexadecimal, 0b and 0B for binary, 0 for octal, decimal
	 *              otherwise).
	 */
	public Mpz(
			String s,
	 ...
	 * <p>
	 * The leading characters are used to determine whether base: 0x and 0X for
	 * hexadecimal, 0b and 0B for binary, 0 for octal, and decimal otherwise.
	 */
	public Mpz(
			String s) {
		init_set(s, 0);
	...
	/**
	 * Constructs a number from a unsigned stored in a big-endian byte array and
	 * a sign.
	 */
	public Mpz(
			int sign,
	 ...
	 * <p>
	 * The mantissa m returned is such that 0.5 &lt;= |m| &lt; 1, truncated to
	 * fit double.
	 * <p>
	...
	/**
	 * Stores the quotient of n/d rounded towards +oo into this, and returns the
	 * remainder.
	 * <p>
	 * d must be positive.
	 */
	public native int cdivQ(
			Mpz n,
			int d)
			throws ArithmeticException;
	...
	/**
	 * Stores the remainder of n/d rounded towards +oo into this, and returns
	 * the remainder.
	 * <p>
	 * d must be positive.
	 */
	public native int cdivR(
			Mpz n,
			int d)
			throws ArithmeticException;

	/**
	 * Stores the quotient of n/d rounded towards +oo into this, the remainder
	 * into r, and returns the remainder.
	 * <p>
	 * d must be positive.
	 */
	public native int cdivQR(
			Mpz r,
			Mpz n,
			int d)
			throws ArithmeticException;
	 ...
	 */
	public native void tdivR(
			Mpz n,
			Mpz d)
			throws ArithmeticException;

	/**
	 * Stores the quotient of n/d rounded towards 0 into this and the remainder
	 * into r.
	 */
	public native void tdivQR(
			Mpz r,
			Mpz n,
			Mpz d)
			throws ArithmeticException;
	...
	/**
	 * Stores the quotient of n/d rounded towards 0 into this, and returns the
	 * remainder.
	 * <p>
	 * d must be positive.
	 */
	public native int tdivQ(
			Mpz n,
			int d)
			throws ArithmeticException;
	...
	/**
	 * Stores the remainder of n/d rounded towards 0 into this, and returns the
	 * remainder.
	 * <p>
	 * d must be positive.
	 */
	public native int tdivR(
			Mpz n,
			int d)
			throws ArithmeticException;

	/**
	 * Stores the quotient of n/d rounded towards 0 into this, the remainder
	 * into r, and returns the remainder.
	 * <p>
	 * d must be positive.
	 */
	public native int tdivQR(
			Mpz r,
			Mpz n,
			int d)
			throws ArithmeticException;
	 ...
	 */
	public native boolean isDivisible(
			Mpz d);
	 ...
	 * d must be positive.
	 */
	public native boolean isDivisible2exp(
			int d);
	 ...
	 */
	public native boolean isCongruent(
			Mpz c,
			Mpz d);
	 ...
	 * d must be positive.
	 */
	public native boolean isCongruent2exp(
			Mpz c,
			int d);
	...
	/**
	 * Returns 2 if this is definitely prime, 1 if it is probably prime, and 0
	 * if it is definitely composite.
	 * <p>
	 * r is the number of tests; high numbers increasing the certainty. A value
	 * between 5 and 10 is reasonable.
	 */
	public native int isProbabPrime(
			int r);

	/**
	 * Stores into this the next prime greater than op.
	 * <p>
	 * This uses a probabilistic algorithm so that the chance of getting a
	 * composite are non-null but extremely small.
	 */
	public native void nextPrime(
			Mpz op);
	 ...
	 * <p>
	 * Sets s and t to values such that s*op1+t*op2 = gcd and |s| &lt;= |op2|,
	 * |t| &lt;= |op1|.
	 */
	public native void gcd(
			Mpz s,
	 ...
	 * <p>
	 * Returns true if this inverse exists. Otherwise, returns false; this is
	 * undefined.
	 */
	 ...
	 * b must be odd.
	 */
	static public native int jacobi(
			Mpz b,
			Mpz p);
	 ...
	 */
	static public native int legendre(
			Mpz a,
			Mpz p);

	/** Returns the Jacobi symbol with the Kronecker extension. */
	static public native int kronecker(
			Mpz a,
			Mpz b);
	 ...
	 * n must be positive.
	 */
	public native void lucnum(
			int n);

	/**
	 * Stores into this the n-th Lucas number, and into r the n-1-th Lucas
	 * number.
	 * <p>
	 * n must be positive.
	 */
	 ...
	 * <p>
	 * Returns {@link #infinite} if this is negative.
	 */
	 ...
	 * <p>
	 * Returns {@link #infinite} if this and op have different signs.
	 */
	public native int hamDist(
			Mpz op);
	 ...
	 * <p>
	 * May return {@link #infinite}, if this is negative.
	 */
	public native int scan0(
			int pos);
	 ...
	 * pos must be positive.
	 * <p>
	 * May return {@link #infinite}, if this is positive.
	 */
	public native int scan1(
			int pos);
	...
	private void writeObject(
			...
			throws IOException {
		/*
		 * format: - length (int) - sign (byte) - absolute value, in big-endian
		 * byte order
		 */
	...
	/**
	 * Stores into this a uniformly distributed random number with n bits (i.e.,
	 * in 0..2^n-1).
	 * <p>
	 * n must be positive.
	 */
	public native void randomBits(
			RandState r,
			int n);
	...
	/**
	 * Stores into this a random number with n bits, and long strings of zeros
	 * and ones in its binary representation.
	 * <p>
	 * Useful to trigger corner-case bugs when testing.
	 */
	public native void rRandomBits(
			RandState r,
			int n);
