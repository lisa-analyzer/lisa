...
/**
 * A {@link BaseTaint} implementation with only two levels of taintedness,
 * represented by {@link SimpleTaint}: clean and tainted. As such, this class
 * distinguishes values that are always clean from values that are tainted in at
 * least one execution path, without further separating the latter from values
 * that are tainted on every path. <br/>
 * <br/>
