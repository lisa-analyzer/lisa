...
public class BaseCasesFinder<A extends AbstractLattice<A>,
	...
	public AnalysisState<A> getAbstractResultOf(
		...
		boolean towardsRecursion = CollectionUtils.intersection(recursion.getMembers(), call.getTargetedCFGs())
				.isEmpty();
		...
		} else if (towardsRecursion)
			// FIXME: we might come up with a strategy here
			throw new SemanticException("Calls towards a recursion being resolved are not supported yet");
