...
public class Trend implements BaseNonRelationalValueDomain<Trend> {
	...
	public static Trend generateTrendNonDecIfGt(
		...
		if (isEqual)
			return STABLE;
		else if (isGreater || isGreaterOrEq)
			return NON_DEC;
		else if (isLess || isLessOrEq)
			return NON_INC;
	...
	public static Trend generateTrendIncIfBetween(
		...
		if (isEqualA || isEqualB)
			...
			return DEC;
		else if (isLessOrEqA || isGreaterOrEqB)
			return NON_INC;
		else if (isNotEqA && isNotEqB)
			return NON_STABLE;
	...
	public static Trend generateTrendNonDecIfBetween(
		...
		if (isEqualA || isEqualB)
			return STABLE;
				...
				&& (isLessB || isLessOrEqB))
			return NON_DEC;
		else if (isLessA || isLessOrEqA || isGreaterB || isGreaterOrEqB)
			return NON_INC;
