...
public class DifferenceBoundMatrix
        implements BaseLattice<DifferenceBoundMatrix>, ValueDomain<DifferenceBoundMatrix> {
    ...
    public DifferenceBoundMatrix(MathNumber[][] matrix, Map<Identifier, Integer> variableIndex) {
        this.matrix = matrix;
        this.variableIndex = variableIndex;
    }

    private MathNumber[][] copyMatrix(MathNumber[][] source) {
...
public DifferenceBoundMatrix bottom() {
     ...
     */
    @Override
    public boolean lessOrEqualAux(DifferenceBoundMatrix other) throws SemanticException {
        if (this.isBottom()) {
            return true;
        }
     ...
     */
    @Override
    public DifferenceBoundMatrix lubAux(DifferenceBoundMatrix other) throws SemanticException {
     ...
     */
    @Override
    public DifferenceBoundMatrix glbAux(DifferenceBoundMatrix other) throws SemanticException {
        // first check if two dbm are the same
        for (Map.Entry<Identifier, Integer> entry : variableIndex.entrySet()) {
            if (!other.variableIndex.containsKey(entry.getKey())) {
                throw new SemanticException("Variable are not the same");
    ...
    @Override
    public DifferenceBoundMatrix assign(Identifier id, ValueExpression expression, ProgramPoint pp,
        ...
        // Work on a local matrix reference so we can add a variable and still continue
            ...
            for (int i = 0; i < newSize; i++) {
                for (int j = 0; j < newSize; j++) {
                    if (i < matrix.length && j < matrix.length) {
                        ...
                        newMatrix[i][j] = MathNumber.ZERO; // distance to self is 0
                    } else {
                        newMatrix[i][j] = MathNumber.PLUS_INFINITY; // unknown distances are +inf
                    }
                }
            }
    ...
    @Override
    public DifferenceBoundMatrix smallStepSemantics(ValueExpression expression, ProgramPoint pp, SemanticOracle oracle)
            throws SemanticException {
        return this;
    }

    @Override
    public DifferenceBoundMatrix assume(ValueExpression expression, ProgramPoint src, ProgramPoint dest,
            SemanticOracle oracle) throws SemanticException {
        ...
        if (sub.getOperator().equals(NumericNonOverflowingSub.INSTANCE)) {
            ...
            // Objective: call addConstraint(Identifier a, Identifier b, MathNumber c)
    ...
    @Override
    public DifferenceBoundMatrix forgetIdentifier(Identifier id) throws SemanticException {
    ...
    @Override
    public DifferenceBoundMatrix forgetIdentifiersIf(Predicate<Identifier> test) throws SemanticException {
    ...
    @Override
    public DifferenceBoundMatrix pushScope(ScopeToken token) throws SemanticException {
        return this;
    }

    @Override
    public DifferenceBoundMatrix popScope(ScopeToken token) throws SemanticException {
        return this;
    }
    ...
    /*
     * The formal definition of πi(m) is as follows:
     * - πi(m) def= ∅ if m• = ⊥DBM (i.e., if the octagon is empty).
     * - [−m•(2i−1)(2i)/2, m•(2i)(2i−1)/2] if m• ≠ ⊥DBM.
     */
    public ValueEnvironment<Interval> toInterval() throws SemanticException {
        ...
        DifferenceBoundMatrix dbm = this.closure(); // apply the closure to ensure the matrix is in normal form
        if (dbm.isBottom()) {
            return env;
        }
     ...
     * - -2 × fst (X(Vk)) if j = 2k, i = 2k − 1
     * - +∞ otherwise
     */
    public static DifferenceBoundMatrix fromIntervalDomain(ValueEnvironment<Interval> env) throws SemanticException {
        ...
        for (int i = 0; i < 2 * n; i++) {
            for (int j = 0; j < 2 * n; j++) {
                if (i == j) {
                    ...
                    matrix[i][j] = MathNumber.PLUS_INFINITY; // unknown distances are +inf
                }
            }
        }
        int index = 0;
        for (Identifier id : env.getKeys()) {
            ...
            if (interval != null) {
                ...
                if (upper != null && !upper.isPlusInfinity()) {
                    matrix[neg][pos] = upper.multiply(new MathNumber(2)); // 2 * snd(X(Vk))
                }
                if (lower != null && !lower.isMinusInfinity()) {
                    matrix[pos][neg] = lower.multiply(new MathNumber(-2)); // -2 * fst(X(Vk))
                }
            }
            index++;
        }
    ...
    // Convert a generic object (coming from Constant.getValue()) into MathNumber,
    // if possible
    private MathNumber toMathNumber(Object v) {
        if (v == null)
            return null;
    ...
    private int idToPos(Identifier id, Map<Identifier, Integer> varIndex) {
    ...
    private int idToNeg(Identifier id, Map<Identifier, Integer> varIndex) {
        if (!varIndex.containsKey(id))
            throw new IllegalArgumentException("Identifier " + id + " not found in variable index.");
        int i = varIndex.get(id);
        int result = 2 * i + 1;
    ...
    private DifferenceBoundMatrix addConstraint(Identifier a, MathNumber c, boolean isNegated)
            throws SemanticException {
    ...
    private DifferenceBoundMatrix addConstraint(Identifier a, Identifier b, MathNumber c, boolean firstNegated,
            boolean secondNegated) throws SemanticException {
        ...
        /*
         * V_j0 - V_i0 <= c =
         * m[2i-1][2j-1] = min(m[2i-1][2j-1], 2*c) (1)
         * m[2j][2i] = min(m[2j][2i], 2*c) (2)
         * 
         * Vj0 + Vi0 <= c =
         * m[2i][2j-1] = min(m[2i][2j-1], 2*c) (3)
         * m[2j][2i] = min(m[2j][2i], 2*c) (4)
         * 
         * -Vj0 - Vi0 <= c =
         * m[2i-1][2j] = min(m[2i-1][2j], 2*c) (5)
         * m[2j-1][2i] = min(m[2j-1][2i], 2*c) (6)
         */
    ...
    @Override
    public DifferenceBoundMatrix wideningAux(DifferenceBoundMatrix other) throws SemanticException {
        if (other == null) {
            return this;
        }
        ...
        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++) {
                // Widening: if this[i][j] < other[i][j], the constraint is getting weaker, so
                // set to +∞
    ...
    private double resolveVariableExpression(ValueExpression exp) throws MathNumberConversionException {
    ...
    private String hasVariable(ValueExpression exp) {
    ...
    private double resolveCostantExpression(ValueExpression exp) throws MathNumberConversionException {
        // Base cases
        if (isDouble(exp.toString())) {
     ...
     * such as Vi = -Vi.
     */
    public void fourthCaseAssignement(MathNumber[][] curMatrix, int newVariableIndex, ValueExpression expression,
            Identifier id) {
    ...
    /**
     * Handles the assignment case where a variable is assigned the value of another
     * variable plus a constant: Vi = Vj + c, where i != j.
     * Updates the matrix to link the variables and incorporate the constant offset.
     */
    public void thirdCaseAssignement(MathNumber[][] curMatrix, int newVariableIndex, ValueExpression expression,
            Identifier id, double offset, boolean isOffset) {
    ...
    /**
     * Handles the assignment case where a variable is incremented by a constant: Vi
     * = Vi + c.
     * Adjusts the matrix entries to account for the addition to the variable's
     * bounds.
     */
    public void secondCaseAssignement(MathNumber[][] curMatrix, int newVariableIndex, ValueExpression expression,
            Identifier id, double offset, boolean isOffset) {
        MathNumber value = new MathNumber(0);
    ...
    /**
     * Handles the assignment case where a variable is assigned a constant value: Vi
     * = c.
     * Updates the DBM matrix to reflect the new bounds for the variable by setting
     * appropriate constraints.
     */
    public void firstCaseAssignement(MathNumber[][] curMatrix, int newVariableIndex, ValueExpression expression,
            Identifier id, Map<Identifier, Integer> varIndex) {
    ...
    public double resolveStringMath(String expr) {
        if (isDouble(expr)) {
    ...
    public boolean isValidConstraint(MathNumber[][] matrix) {
        for (int i = 0; i < matrix.length; i++) {
            for (int j = 0; j < matrix.length; j++) {
    ...
    boolean verifySingleIndex(MathNumber[][] mat, int indexI, MathNumber valueI) {
        int I2 = indexI * 2 - 1;
        int I2_Minus1 = indexI * 2 - 2;
