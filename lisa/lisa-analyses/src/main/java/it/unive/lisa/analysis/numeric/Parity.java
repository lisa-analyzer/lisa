 ...
 * Variables are approximated by their parity only, and operators are evaluated
 * according to the usual parity rules (e.g., the sum of two values with the
 * same parity is even), falling back to top whenever the parity of the result
 * cannot be determined precisely (e.g., the sum of an even and an odd value).
 * When a
 * {@link it.unive.lisa.analysis.combination.constraints.WholeValueAnalysis} is
 * available, some operators that would otherwise be approximated to top (such
 * as {@link StringLength}) are instead refined by querying the constraints
