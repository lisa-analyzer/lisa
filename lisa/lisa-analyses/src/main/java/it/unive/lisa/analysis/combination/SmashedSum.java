 ...
 */
public class SmashedSum<S extends StringDomain<S>>
	 ...
	 */
	public SmashedSum(Interval intValue, S stringValue, Satisfiability boolValue) {
	...
	public SmashedSum<S> lubAux(SmashedSum<S> other) throws SemanticException {
		return new SmashedSum<S>(intValue.lub(other.intValue), stringValue.lub(other.stringValue), boolValue.lub(other.boolValue));
	}

	@Override
	public SmashedSum<S> wideningAux(SmashedSum<S> other) throws SemanticException {
		return new SmashedSum<S>(intValue.widening(other.intValue), stringValue.widening(other.stringValue), boolValue.widening(other.boolValue));

	}

	@Override
	public boolean lessOrEqualAux(SmashedSum<S> other) throws SemanticException {
		return intValue.lessOrEqual(other.intValue) && stringValue.lessOrEqual(other.stringValue) && boolValue.lessOrEqual(other.boolValue);
	}
				...
				if (b >= 0)
					for (long e : end) {
						if (b < e)
							...
							temp = partial.lub(this.stringValue.evalNonNullConstant(new Constant(Untyped.INSTANCE, "", SyntheticLocation.INSTANCE), null, oracle));
	...
	public StructuredRepresentation representation() {
		...
		return new StringRepresentation(
				"(" + intValue.representation().toString() + ", " + stringValue.representation().toString() + ", " + boolValue.representation().toString() + ")");
	}

	private boolean sameKind(SmashedSum<S> other) {
    ...
    private SmashedSum<S> mkSmashedValue(Satisfiability boolValue) {
