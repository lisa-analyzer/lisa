...
public class IndexFinder {
	...
	public static Pair<Integer, Integer> findIndexesOf(RegexAutomaton automaton, RegexAutomaton search)
		...
		Set<Transition<RegularExpression>> topTransitions = exploded.getTransitions().parallelStream()
				.filter(t -> t.getSymbol() == TopAtom.INSTANCE).collect(Collectors.toSet());
			...
			if (s.isEmpty())
				...
				return Pair.of(0,
						automaton.acceptsTopEventually() || automaton.hasCycle() ? null
								: automaton.lenghtOfLongestString());
			...
			for (List<State> p : allPaths) {
				...
				for (Vector<Transition<RegularExpression>> match : matching)
					...
					if ((index = p.indexOf(match.firstElement().getSource())) != -1
						...
						int i = index;
						long tops = topTransitions.parallelStream().map(t -> Pair.of(t.getSource(), t.getDestination()))
