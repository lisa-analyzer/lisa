/*
 * Texpr0BinNode.java APRON Library / Java Apron binding Copyright (C) Antoine
 * Mine' 2010
 */
...
/**
 * Class of binary operator nodes in {@link apron.Texpr0Node} level 0 expression
 * trees.
 */
public class Texpr0BinNode
	...
	static public int opPrecedence(
			int op) {
		switch (op) {
		case OP_ADD:
		case OP_SUB:
	...
	/**
	 * Creates a new binary operator node, with the specified operator, rounding
	 * type, rounding direction, left and right arguments.
	 * <p>
	 * The subtree arguments are referenced, not copied.
	 */
	public Texpr0BinNode(
			int o,
			int rtyp,
			int rdi,
			Texpr0Node lAr,
	...
	/**
	 * Creates a new binary operator node, with the specified operator, and left
	 * and right arguments, and no rounding.
	 * <p>
	 * The subtree arguments are referenced, not copied.
	 */
	public Texpr0BinNode(
	...
	public String toString(
		...
		if (lArg.getPrecedence() < opPrecedence(op))
			l = "(" + l + ")";
		if (rArg.getPrecedence() <= opPrecedence(op))
			r = "(" + r + ")";
		return l + " " + o + suf + " " + r;
	}
	...
	public void setRoundingDirection(
			int d) {
		rdir = d;
	}
	...
	public Texpr0Node removeDimensions(
			Dimchange c) {
	...
	public boolean isEqual(
		...
		if (t instanceof Texpr0BinNode) {
			...
			return op == tt.op && rtype == tt.rtype && rdir == tt.rdir &&
