/*
 * Texpr0Node.java APRON Library / Java Apron binding Copyright (C) Antoine
 * Mine' 2010
 */
 ...
 * <p>
 * A Texpr0Node represents an expression tree, which can be constructed and
 * inspected in Java. Each node is actually a binary operator
 * ({@link apron.Texpr0BinNode}), a unary operator ({@link apron.Texpr0UnNode}),
 * a variable represented by dimension ({@link apron.Texpr0DimNode}), or a
 * constant ({@link apron.Texpr0CstNode}).
 * <p>
 * An expression tree must be converted to and from a {@link apron.Texpr0Intern}
 * to be used in abstract domain transfer functions.
 */
public abstract class Texpr0Node
	 ...
	 */
	static public Texpr0Node fromLinexpr0(
			Linexpr0 c) {
		Texpr0Node r = null;
		Coeff cst = c.getCst();
		if (!cst.isZero())
			r = new Texpr0CstNode(cst.copy());
		Linterm0[] l = c.getLinterms();
		for (int i = 0; i < l.length; i++) {
			if (l[i].coeff.isZero())
				continue;
			Texpr0Node rr = new Texpr0BinNode(Texpr0BinNode.OP_MUL,
					new Texpr0CstNode(l[i].coeff.copy()),
					new Texpr0DimNode(l[i].dim));
			r = (r == null) ? rr : new Texpr0BinNode(Texpr0BinNode.OP_ADD, r, rr);
		}
		if (r == null)
	 ...
	 */
	public abstract Texpr1Node toTexpr1Node(
			Environment e);
	...
	public abstract boolean hasDim(
			int dim);

	/** Used internally by getDims. */
	protected abstract void fillDims(
			boolean[] ar);
	...
	/**
	 * Whether the expression is a polynomial fraction, with interval or scalar
	 * coefficients, and there is no rounding.
	 */
	...
	/**
	 * Substitutes all occurrences of the dimension dim with the expression tree
	 * t.
	 * <p>
	 * this is modified in-place.
	 * <p>
	 * As some nodes can change their kind (a node for a dimension dim becomes
	 * the root of the tree t), a new root node is returned.
	 */
	public abstract Texpr0Node substitute(
			int dim,
			Texpr0Node t);

	/**
	 * Returns a copy of this where all occurrences of the dimension dim are
	 * substituted with the expression tree t.
	 */
	public Texpr0Node substituteCopy(
			int dim,
			Texpr0Node t) {
	...
	public Texpr0Node addDimensionsCopy(
			Dimchange c) {
		Texpr0Node x = this.deepCopy();
		x.addDimensions(c);
		return x;
	 ...
	 * this is modified in-place.
	 * <p>
	 * As some nodes can change their kind (removed dimensions become [-oo,+oo]
	 * constants), a new root node is returned.
	 */
	public abstract Texpr0Node removeDimensions(
			Dimchange c);

	/** Returns a copy of this with some dimensions removed. */
	public Texpr0Node removeDimensionsCopy(
			Dimchange c) {
		Texpr0Node x = this.deepCopy();
	...
	public abstract void permuteDimensions(
			Dimperm p);

	/** Returns a copy of this with some dimensions permuted. */
	public Texpr0Node permuteDimensionsCopy(
			Dimperm p) {
		Texpr0Node x = this.deepCopy();
	...
	static public String suffixRType(
			int rtype) {
		switch (rtype) {
		case RTYPE_REAL:
			return "";
		...
		case RTYPE_QUAD:
			return "q";
		default:
			throw new IllegalArgumentException("unknown rounding type: " + rtype);
		}
	}
	...
	static public String suffixRDir(
			int rdir) {
		switch (rdir) {
		case RDIR_NEAREST:
			return "n";
		...
		case RDIR_RND:
			return "?";
		default:
			throw new IllegalArgumentException("unknown rounding direction: " + rdir);
		}
	}
