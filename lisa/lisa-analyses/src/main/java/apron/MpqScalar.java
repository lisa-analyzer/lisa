/*
 * MpqScalar.java APRON Library / Java Apron binding Copyright (C) Antoine Mine'
 * 2010
 */
 ...
 * rational.
 * <p>
 * Note: infinities are represented by using a null denominator. This may cause
 * problems if manipulated by {@link gmp.Mpq Mpq} functions.
 */
public class MpqScalar
	...
	public int toDouble(
		...
		int i = isInfty();
		if (i > 0) {
			r[0] = Double.POSITIVE_INFINITY;
			return 0;
		}
		if (i < 0) {
			r[0] = Double.NEGATIVE_INFINITY;
			return 0;
		}
		Mpfr x = new Mpfr(53);
	...
	public int cmp(
		...
		if (x instanceof MpfrScalar)
			return -((MpfrScalar) x).val.cmp(val);
	...
	public boolean isEqual(
		...
		if (i1 != 0)
			return true;
		if (x instanceof MpqScalar)
			return val.isEqual(((MpqScalar) x).val);
		if (x instanceof MpfrScalar)
			return ((MpfrScalar) x).val.cmp(val) == 0;
		if (x instanceof DoubleScalar) {
			// converts x to Mpfr exactly, and compare to this
			Mpfr xx = new Mpfr(53);
			xx.set(((DoubleScalar) x).val, Mpfr.RNDN);
