/*
 * Texpr1Node.java APRON Library / Java Apron binding Copyright (C) Antoine
 * Mine' 2010
 */
 ...
 * <p>
 * A Texpr1Node represents an expression tree, which can be constructed and
 * inspected in Java. Each node is actually a binary operator
 * ({@link apron.Texpr1BinNode}), a unary operator ({@link apron.Texpr1UnNode}),
 * a variable represented by name ({@link apron.Texpr1VarNode}), or a constant
 * ({@link apron.Texpr1CstNode}).
 * <p>
 * An expression tree must be converted to and from a {@link apron.Texpr1Intern}
 * to be used in abstract domain transfer functions. Unlike other level 1
 * objects, level 1 concrete expression trees are not defined with respect to a
 * given environment. The environment is specified during the conversion to an
 * opaque level 1 expression tree.
 */
public abstract class Texpr1Node
	 ...
	 */
	static public Texpr1Node fromLinexpr1(
			Linexpr1 c) {
		Texpr1Node r = null;
		Coeff cst = c.getCst();
		if (!cst.isZero())
			r = new Texpr1CstNode(cst.copy());
		Linterm1[] l = c.getLinterms();
		for (int i = 0; i < l.length; i++) {
			if (l[i].coeff.isZero())
				continue;
			Texpr1Node rr = new Texpr1BinNode(Texpr1BinNode.OP_MUL,
					new Texpr1CstNode(l[i].coeff.copy()),
					new Texpr1VarNode(l[i].var));
			r = (r == null) ? rr : new Texpr1BinNode(Texpr0BinNode.OP_ADD, r, rr);
		}
		if (r == null)
	 ...
	 */
	public abstract Texpr0Node toTexpr0Node(
			Environment e);
	...
	protected abstract void fillVars(
			HashSet<Var> set);
	 ...
	 * <p>
	 * As some nodes can change their kind (a node for a variable var becomes
	 * the root of the tree t), a new root node is returned.
	 */
	public abstract Texpr1Node substitute(
			Var var,
			Texpr1Node t);

	public Texpr1Node substitute(
			String var,
			Texpr1Node t) {
	 ...
	 */
	public Texpr1Node substituteCopy(
			Var var,
			Texpr1Node t) {
