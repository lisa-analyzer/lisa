/*
 * Tcons0.java APRON Library / Java Apron binding Copyright (C) Antoine Mine'
 * 2010
 */
 ...
 * <p>
 * A Tcons0 represents a constraint over variables denoted by dimension (i.e.,
 * integer starting from 0). It has a left member (which is a
 * {@link apron.Texpr0Intern} level 0 opaque expression), a constraint operator
 * (=, &gt;=, &gt;, &lt;&gt;, or = modulo), a right member which is always 0,
 * and an optional constant modulo factor (for = modulo constraints only).
 */
public class Tcons0
		implements
		Cloneable,
	...
	/**
	 * Constructs a constraint with given kind, e as left member, and s as
	 * modulo factor.
	 * <p>
	 ...
	 * Constructs a constraint with given kind and e as left member.
	 * <p>
	 * e is converted to an opaque {@link apron.Texpr0Intern} Apron expression
	 * tree.
	 */
	public Tcons0(
			int k,
	...
	public String toString(
		...
		case DISEQ:
			return s + " <> 0";
		case EQMOD:
			return s + " = 0 mod " + ((scalar == null) ? "<null>" : scalar.toString());
		default:
			throw new IllegalArgumentException("unknown constraint kind");
		}
	}
	 ...
	 * this is not changed.
	 */
	public Tcons0 addDimensionsCopy(
			Dimchange c) {
	 ...
	 * this is not changed.
	 */
	public Tcons0 removeDimensionsCopy(
			Dimchange c) {
		Tcons0 l = new Tcons0(this);
	 ...
	 * this is not changed.
	 */
	public Tcons0 permuteDimensionsCopy(
			Dimperm p) {
		Tcons0 l = new Tcons0(this);
