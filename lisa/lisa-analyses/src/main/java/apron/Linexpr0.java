/*
 * Linexpr0.java APRON Library / Java Apron binding Copyright (C) Antoine Mine'
 * 2010
 */
 ...
 * <p>
 * A Linexpr0 is a linear expression with {@link apron.Coeff} coefficients and
 * variables denoted by dimension (i.e., integer starting from 0).
 * <p>
 * A Linexpr0 object encapsulates an {@code ap_linexpr0_t} Apron object
 * allocated in the C heap.
 * <p>
 * A Linexpr0 can be either sparse or dense. A dense Linexpr0 is similar to an
 * array of coefficients. Its size is set a creation time, but can be manually
 * changed. It has a coefficient for each dimension, from 0 to size-1. A sparse
 * Linexpr0 needs not be continuous: omitted dimensions implicitly have a 0
 * coefficient. Also, sparse Linexpr0 grow automatically on-demand.
 */
public class Linexpr0
	...
	/**
	 * Creates a new sparse Linexpr0 with the specified dimension and constant
	 * coefficient.
	 * <p>
	 * All coefficients are copied.
	 */
	...
	/**
	 * Creates a new dense Linexpr0 with the specified dimension and constant
	 * coefficients.
	 * <p>
	 ...
	 * <p>
	 * Trying to set a dimension greater than the size is an error for a dense
	 * Linexpr0. For sparse Linexpr0, if there is no coefficient for dim, one is
	 * created.
	 * <p>
	 ...
	 * <p>
	 * The array is sorted in increasing order of dimension. For sparse
	 * Linexpr0, some dimensions may be missing.
	 */
	 ...
	 * <p>
	 * The size is fixed at creation time for dense Linexpr0, but changes
	 * dynamically for sparse Linexpr0.
	 **/
	...
	public String toString(
		...
		for (int i = 0; i < x.length; i++) {
			...
			if (c.isZero())
				continue;
			if (!first && !(c instanceof Scalar && ((Scalar) c).sgn() < 0))
				buf.append(" +");
			else
				buf.append(" ");
			...
			if (names == null) {
				...
				buf.append(names[x[i].dim].toString());
			first = false;
		}
		Coeff c = getCst();
		if (!c.isZero()) {
	...
	/**
	 * Whether the expression only depends on real-valued dimensions (dimensions
	 * intdim and greater).
	 */
	...
	/**
	 * Whether the expression has only scalar coefficients, except maybe for the
	 ...
	 * <p>
	 * Transforms intervals into scalars when possible (when their bounds are
	 * equal). For sparse Linexpr0, also removes zero coefficients.
	 */
	 ...
	 * this is not changed.
	 */
	public Linexpr0 addDimensionsCopy(
			Dimchange c) {
	 ...
	 * this is not changed.
	 */
	public Linexpr0 permuteDimensionsCopy(
			Dimperm p) {
		Linexpr0 l = new Linexpr0(this);
