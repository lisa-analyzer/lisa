/*
 * Abstract1.java APRON Library / Java Apron binding Copyright (C) Antoine Mine'
 * 2010
 */
 ...
 * <p>
 * A level 1 numerical abstract value object represents a set of points in an
 * environment.
 * <p>
 * An Abstract1 is implemented as a pair containing an {@link apron.Abstract0}
 * and an {@link apron.Environment}, manipulated in conjunction. Direct access
...
public class Abstract1
	...
	private Abstract0 makeAbstract0Opt(
			Abstract1 x) {
		if (x == null)
			return null;
		checkEnv(x.env);
	...
	static private Abstract0[] makeAbstract0Array(
			Abstract1[] ar) {
	...
	private Lincons0[] makeLincons0Array(
			Lincons1[] ar) {
	...
	private Tcons0[] makeTcons0Array(
			Tcons1[] ar) {
	...
	private Generator0[] makeGenerator0Array(
			Generator1[] ar) {
	...
	private Linexpr0[] makeLinexpr0Array(
			Linexpr1[] ar) {
	...
	private Texpr0Intern[] makeTexpr0InternArray(
			Texpr1Intern[] ar) {
	 ...
	 * <p>
	 * The element is such that vars[i] is in interval box[i]. Variables not in
	 * vars have interval [-oo,+oo].
	 * <p>
	 ...
	 * <p>
	 * Warning: it is dangerous to add, remove, or permute dimensions as it may
	 * desynchrnonize the Abstract0 dimensions form the environment.
	 */
	...
	/**
	 * Returns (an over-approximation of) the set of values the variable var can
	 * take on this.
	 */
	public Interval getBound(
			Manager man,
	 ...
	 * <p>
	 * The variable order in the returned array is that of the environment,
	 * i.e., {@code getEnvironment().getVars()}.
	 */
	public Interval[] toBox(
			Manager man)
			throws ApronException {
	 ...
	 * the element is definitely empty.
	 */
	...
	/**
	 * Whether the projection of this on variable var is definitely included in
	...
	public boolean isDimensionUnconstrained(
			Manager man,
			String var)
			throws ApronException {
	...
	/**
	 * Whether the variable var is definitely unbounded in this (true) or may be
	 * bounded (false).
	 * <p>
	 * If {@link apron.Manager#wasExact} returns true, then false indicates that
	 ...
	 * this, dest, and the array must have the same environment.
	 */
	public Abstract1 assignCopy(
			Manager man,
			Var[] var,
			Linexpr1[] ar,
			Abstract1 dest)
			throws ApronException {
		return new Abstract1(env,
				abs.assignCopy(man, makeDimArray(var), makeLinexpr0Array(ar), makeAbstract0Opt(dest)));
	}
	...
	public Abstract1 assignCopy(
			...
			throws ApronException {
		return new Abstract1(env,
				abs.assignCopy(man, makeDimArray(var), makeTexpr0InternArray(ar), makeAbstract0Opt(dest)));
	}
	 ...
	 * this, dest, and the array must have the same environment.
	 */
	public Abstract1 substituteCopy(
			Manager man,
			Var[] var,
			Linexpr1[] ar,
			Abstract1 dest)
			throws ApronException {
		return new Abstract1(env,
				abs.substituteCopy(man, makeDimArray(var), makeLinexpr0Array(ar), makeAbstract0Opt(dest)));
	}
	...
	public Abstract1 substituteCopy(
			...
			throws ApronException {
		return new Abstract1(env,
				abs.substituteCopy(man, makeDimArray(var), makeTexpr0InternArray(ar), makeAbstract0Opt(dest)));
	}
	...
	public Abstract1 assignCopy(
		...
		checkEnv(expr.getEnvironment());
		return new Abstract1(env,
				abs.assignCopy(man, env.dimOfVar(var), expr.getLinexpr0Ref(), makeAbstract0Opt(dest)));
	}
	...
	public Abstract1 assignCopy(
		...
		checkEnv(expr.getEnvironment());
		return new Abstract1(env,
				abs.assignCopy(man, env.dimOfVar(var), expr.getTexpr0InternRef(), makeAbstract0Opt(dest)));
	}
	...
	/**
	 * Replaces this with (an over-approximation of) this after substituting var
	 * with expr.
	 * <p>
	 * The result is intersected with dest, if dest is not null.
	 * <p>
	...
	public Abstract1 substituteCopy(
		...
		checkEnv(expr.getEnvironment());
		return new Abstract1(env,
				abs.substituteCopy(man, env.dimOfVar(var), expr.getLinexpr0Ref(), makeAbstract0Opt(dest)));
	}
	...
	public Abstract1 substituteCopy(
		...
		checkEnv(expr.getEnvironment());
		return new Abstract1(env,
				abs.substituteCopy(man, env.dimOfVar(var), expr.getTexpr0InternRef(), makeAbstract0Opt(dest)));
	}

	// Projection
	/////////////
	public Abstract1 forgetCopy(
			Manager man,
			String[] var,
			boolean project)
			throws ApronException {
	 ...
	 * <p>
	 * Variables in this and not in e are removed, while variables in e and not
	 * in this are created.
	 * <p>
	 * project indicates whether added variables are set to 0 (true) or
	 * [-oo,+oo] (false).
	 * <p>
	 ...
	 * this is not modified.
	 */
	public Abstract1 changeEnvironmentCopy(
			Manager man,
			Environment e,
			boolean project)
			throws ApronException {
	...
	private Environment withoutUnconstrained(
			Manager man)
			throws ApronException {
	 ...
	 * this is not modified.
	 */
	public Abstract1 minimizeEnvironmentCopy(
			Manager man)
			throws ApronException {
	 ...
	 */
	public void minimizeEnvironment(
			Manager man)
			throws ApronException {
		changeEnvironment(man, withoutUnconstrained(man), false);
	}

	public Abstract1 renameCopy(
			Manager man,
			String[] org,
			String[] dst)
			throws ApronException {
	...
	/**
	 * Replaces this with the meet of this and op on their least common
	 * environment.
	 */
	public void unify(
			Manager man,
	...
	/**
	 * Returns a new element where variable org is duplicated as dst[0] ...
	 * dst[dst.length-1].
	 * <p>
	 * this is not modified.
	 */
	public Abstract1 expandCopy(
		...
		Environment e;
		if (env.isInt(org))
			e = env.addPerm(dst, null, p);
		else
			e = env.addPerm(null, dst, p);
	...
	public void expand(
		...
		Dimperm[] p = new Dimperm[1];
		if (env.isInt(org))
			env = env.addPerm(dst, null, p);
		else
			env = env.addPerm(null, dst, p);
		abs.permuteDimensions(man, p[0]);
	}

	public Abstract1 foldCopy(
			Manager man,
			String[] var)
			throws ApronException {
	...
	/**
	 * Returns a new element where all the variables var[i] are folded into the
	 * variable var[0].
	 * <p>
	 * The array must contain either only integer-valued or real-valued
	 * variables.
	 * <p>
	 * this is not modified.
	 */
	public Abstract1 foldCopy(
		...
		if (!var0[0].equals(var1[0]))
			r.rename(man, var0, var1);
		return r;
	}
	...
	public void fold(
		...
		Var[] var1 = { var[0] };
		if (env.isInt(var0[0]))
			env = env.remove(var).add(var0, null);
		else
			env = env.remove(var).add(null, var0);
		abs.fold(man, dim);
		if (!var0[0].equals(var1[0]))
			rename(man, var0, var1);
	}
	 ...
	 */
	public Abstract1 wideningThreshold(
			Manager man,
			Abstract1 op,
			Lincons1[] c)
			throws ApronException {
	 ...
	 * managers and identical environments.
	 */
	 ...
	 * {@link #serialize} call. The environments must be identical.
	 */
	static public Abstract1 deserialize(
			Manager man,
			Environment e,
			byte[] buf)
			throws ApronException {
