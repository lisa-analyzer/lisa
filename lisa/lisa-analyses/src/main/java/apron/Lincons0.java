/*
 * Lincons0.java APRON Library / Java Apron binding Copyright (C) Antoine Mine'
 * 2010
 */
 ...
 * <p>
 * A Lincons0 represents a linear constraint over variables denoted by dimension
 * (i.e., integer starting from 0). It has a left member (which is a
 * {@link apron.Linexpr0} level 0 linear expression), a constraint operator (=,
 * &gt;=, &gt;, &lt;&gt;, or = modulo), a right member which is always 0, and an
 * optional constant modulo factor (for = modulo constraints only).
 */
public class Lincons0
	...
	public Lincons0(
		...
		expr = new Linexpr0();
		if (!sat)
			expr.setCst(new DoubleScalar(-1));
		kind = SUPEQ;
		scalar = null;
	}
...
// Get functions
	...
	/**
	 * Whether the constraint only depends on real-valued dimensions (dimensions
	 * intdim and greater).
	 */
	...
	/**
	 * Whether the constraint has only scalar coefficients, except maybe for the
	 ...
	 * <p>
	 * Returns true if the constraint has the form b &gt;= 0 or [a,b] &gt;= 0
	 * with b negative.
	 */
	 ...
	 * this is not changed.
	 */
	public Lincons0 addDimensionsCopy(
			Dimchange c) {
	 ...
	 * this is not changed.
	 */
	public Lincons0 permuteDimensionsCopy(
			Dimperm p) {
		Lincons0 l = new Lincons0(this);
