/*
 * Manager.java APRON Library / Java Apron binding Copyright (C) Antoine Mine'
 * 2010
 */
 ...
 * <p>
 * Each abstract domain subclasses the Manager class to provide a Manager
 * factory for the domain. The Manager is itself a factory for abstract elements
 * in the corresponding abstract domain: abstract elements can be constructed
 * and manipulated by passing the Manager instance to the
 * {@link apron.Abstract0} and {@link apron.Abstract1} classes.
 * <p>
 * A Manager stores a set of abstract domain configuration parameters to tune
 * the cost/precision of the instance of the abstract domain.
 * <p>
 * Finally, a Manager stores thread-local information. It is safe to call all
 * Apron methods in concurrent threads as long as all arguments (including this,
 * and including the Manager argument) are distinct in all threads.
 * <p>
 * A Manager object encapsulates an {@code ap_manager_t} Apron object allocated
 * in the C heap.
 */
public class Manager {
	...
	/**
	 * Managers can only be constructed from an abstract domain Manager
	 * subclass.
	 */
	protected Manager() {
	}
	...
	/**
	 * Returns the name of the numerical abstract domain library that created
	 * the manager.
	 */
	public native String getLibrary();

	/**
	 * Returns the version of the numerical abstract domain library that created
	 * the manager.
	 */
	 ...
	 * ({@link #FUNID_COPY}, etc.).
	 */
	public native int getMaxObjectSize(
			int funid);
	 ...
	 * ({@link #FUNID_COPY}, etc.)..
	 */
	public native boolean getFlagExactWanted(
			int funid);
	 ...
	 * ({@link #FUNID_COPY}, etc.).
	 */
	public native boolean getFlagBestWanted(
			int funid);
	 ...
	 * ({@link #FUNID_COPY}, etc.).
	 * <p>
	 * 0 is the default. Negative numbers indicate less precision, while
	 * positive numbers indicate more precision.
	 * <p>
	 ...
	 * {@link apron.Abstract0#getSize}.
	 * <p>
	 * 0 indicates no limit.
	 */
	public native void setMaxObjectSize(
			int funid,
			int size);

	/**
	 * Whether the given operation ({@link #FUNID_COPY}, etc.) should update the
	 * exactness flag.
	 * <p>
	 * As computing the exactness information may be costly, it is disabled by
	 * default for all operations.
	 */
	public native void setFlagExactWanted(
			int funid,
			boolean wanted);
	 ...
	 * best-precision flag.
	 * <p>
	 * As computing the best-precision information may be costly, it is disabled
	 * by default for all operations.
	 */
	public native void setFlagBestWanted(
			int funid,
			boolean wanted);
	...
	/**
	 * Whether the last operation with {@link #setFlagBestWanted} set returned a
	 * result which is optimal with respect to the abstraction.
	 */
	public native boolean wasBest();

	/**
	 * Gets the currently chosen preferred scalar type ({@link #SCALAR_DOUBLE},
	 * etc.).
	 */
	 ...
	 */
	public native void setPreferedScalarType(
			int t);
