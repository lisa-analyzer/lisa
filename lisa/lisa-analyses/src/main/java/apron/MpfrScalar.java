/*
 * MpfrScalar.java APRON Library / Java Apron binding Copyright (C) Antoine
 * Mine' 2010
 */
...
/**
 * Class of Scalar objects backed up by a {@link gmp.Mpfr Mpfr} multi-precision
...
public class MpfrScalar
	 ...
	 * Creates a new Mpfr Scalar holding a copy of x.
	 * <p>
	 * x is rounded to the default Mpfr precision (see
	 * {@link gmp.Mpfr#setDefaultPrec gmp.Mpfr.setDefaultPrec}) in the direction
	 * specified by round.
	 */
	public MpfrScalar(
			double x,
	...
	public int cmp(
		...
		if (x instanceof MpfrScalar)
			return val.cmp(((MpfrScalar) x).val);
		if (x instanceof DoubleScalar)
			return val.cmp(((DoubleScalar) x).val);
	...
	public boolean isEqual(
			Scalar x) {
		if (x instanceof MpfrScalar)
			return val.isEqual(((MpfrScalar) x).val);
		if (x instanceof DoubleScalar)
			return val.cmp(((DoubleScalar) x).val) == 0;
		if (x instanceof MpqScalar) {
			int i1 = isInfty();
			int i2 = x.isInfty();
			if (i1 != i2)
