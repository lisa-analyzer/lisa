/*
 * Linexpr1.java APRON Library / Java Apron binding Copyright (C) Antoine Mine'
 * 2010
 */
 ...
 * <p>
 * A Linexpr1 is a linear expression with {@link apron.Coeff} coefficients and
 * variables denoted by names in an environment.
 * <p>
 * A Linexpr1 is implemented as a pair containing a (sparse or dense)
 * {@link apron.Linexpr0} and an {@link apron.Environment}, manipulated in
...
public class Linexpr1
	...
	/**
	 * This function is used internally to construct a level 1 expression by
	 * referencing a level 0 expression.
	 * <p>
	 * The level 0 expression should no longer be used afterwards.
	 */
	static public Linexpr1 make(
			Environment v,
			Linexpr0 x) {
	 ...
	 * <p>
	 * size is only an initial guess; the expression will grow dynamically when
	 * needed.
	 * <p>
	...
	/**
	 * Creates a new sparse expression initialized with the corresponding terms
	...
	/**
	 * Returns a (read-only) reference to the Linexpr0 underlying this
	 * expression.
	 * <p>
	 * Warning: it is dangerous to add coefficients, resize the expression, add,
	 * remove, or permute dimensions as it may desynchrnonize the Linexpr0
	 ...
	 * <p>
	 * All integer-valued variables appear before real-valued variables.
	 * Integer-valued and real-valued variables appear in increasing
	 * lexicographic order (i.e., the dimension order).
	 * <p>
	 * For sparse expressions, some variable may be missing (and should be
	 * considered implicitly as 0).
	 */
	 ...
	 * Compares this and x.
	 * <p>
	 * The order is a lexicographic order of {@link apron.Environment#cmp} and
	 * {@link apron.Linexpr0#cmp}.
	 */
	public int cmp(
			Linexpr1 x) {
		int c = env.cmp(x.env);
		if (c == 0)
			return expr.cmp(x.expr);
		else
			return c;
	}
	 ...
	 */
	public Linexpr1 extendEnvironmentCopy(
			Environment e) {
