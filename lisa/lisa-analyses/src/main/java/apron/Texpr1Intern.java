/*
 * Texpr1Intern.java APRON Library / Java Apron binding Copyright (C) Antoine
 * Mine' 2010
 */
 ...
 * <p>
 * A Texpr1Intern represents an expression tree, with unary and binary operator
 * nodes, and {@link apron.Coeff} constant and variables (represented by names
 * in an environment) leaves.
 * <p>
 * A Texpr1Intern is implemented as a pair containing an opaque level 0 tree
 * {@link apron.Texpr0Intern} and an {@link apron.Environment}, manipulated in
 * conjunction. Direct access to the fields is prohibited, to avoid
 * desynchronizing them.
 * <p>
 * A Texpr1Intern encapsulates an Apron {@code ap_texpr0_t} object, an opaque
 * object that can be passed to and retreived from Apron abstract domains.
...
public class Texpr1Intern
	...
	/**
	 * This function is used internally to construct a new level 1 expression
	 * from a level 0 expression.
	 * <p>
	 * The level 0 expression should no longer be used afterwards.
	 */
	static public Texpr1Intern make(
			Environment v,
			Texpr0Intern x) {
	 ...
	 * <p>
	 * As concrete Texpr1Node trees do not specify an environment, it must be
	 * passed as argument to the conversion method.
	 */
	public Texpr1Intern(
			Environment e,
			Texpr1Node t) {
	...
	/**
	 * Returns a (read-only) reference to the Texpr0Intern underlying this
	 * expression.
	 * <p>
	 * Warning: it is dangerous to add, remove, or permute dimensions as it may
	 * desynchrnonize the Texpr0Intern dimensions form the environment.
	 */
	...
	/**
	 * Substitutes all occurrences of the variable var with the expression tree
	 * t.
	 * <p>
	...
	public Texpr0Intern substituteCopy(
			String var,
			Texpr1Intern t) {
		Var vn = new StringVar(var);
	...
	/**
	 * Returns a copy of this where all occurrences of the variable var are
	 ...
	 * this is not modified.
	 */
	public Texpr1Intern extendEnvironmentCopy(
			Environment e) {
